# Fernzugriff und sicherer Dateiübertragung

**Sprachen:** [English](README.md) · [简体中文](README_zh.md) · [Español](README_es.md) · [Français](README_fr.md) · [Deutsch](README_de.md) · [日本語](README_ja.md) · [Русский](README_ru.md) · [한국어](README_ko.md) · [Português](README_pt.md)

<p align="center">
  <a href="https://labex.io/de/courses/remote-access-and-secure-file-transfer">
    <img src="https://course-cover.labex.io/remote-access-and-secure-file-transfer.png?lang=de" alt="Fernzugriff und sicherer Dateiübertragung">
  </a>
</p>

Lernen Sie Fernzugriff und sichere Dateiübertragung kennen. Dieses Modul behandelt die Verbindung über SSH, SSH-Schlüsselauthentifizierung, sicheres Kopieren von Dateien mit SCP und RSYNC, interaktive Dateiübertragung mit SFTP, SSH Config und Aliase. Sie werden diese wesentlichen Linux-Fähigkeiten durch praktische Labs und reale Herausforderungen meistern.

[Kurs auf LabEx starten](https://labex.io/de/courses/remote-access-and-secure-file-transfer)

## Übungen

|   Index | Name                                       | Schwierigkeit   | Übung                                                                                                                                                       |
|---------|--------------------------------------------|-----------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------|
|       1 | Verbindung via SSH herstellen              | Anfänger        | <a target='_blank' href='https://labex.io/de/labs/connecting-via-ssh-636453?course=remote-access-and-secure-file-transfer'>Lab starten</a>                  |
|       2 | SSH-Schlüsselauthentifizierung             | Anfänger        | <a target='_blank' href='https://labex.io/de/labs/ssh-key-authentication-636457?course=remote-access-and-secure-file-transfer'>Lab starten</a>              |
|       3 | Sichere Dateiübertragung mit SCP und RSYNC | Anfänger        | <a target='_blank' href='https://labex.io/de/labs/secure-file-copy-with-scp-and-rsync-636455?course=remote-access-and-secure-file-transfer'>Lab starten</a> |
|       4 | Interaktive Dateiübertragung mit SFTP      | Anfänger        | <a target='_blank' href='https://labex.io/de/labs/interactive-file-transfer-with-sftp-636454?course=remote-access-and-secure-file-transfer'>Lab starten</a> |
|       5 | SSH-Konfiguration und Aliase               | Anfänger        | <a target='_blank' href='https://labex.io/de/labs/ssh-config-and-aliases-636456?course=remote-access-and-secure-file-transfer'>Lab starten</a>              |
|       6 | Mission: Sichere Bereitstellung            | Anfänger        | <a target='_blank' href='https://labex.io/de/labs/secure-deployment-mission-636637?course=remote-access-and-secure-file-transfer'>Challenge starten</a>     |

## About LabEx

<div align="left"><p><a href="https://labex.io"><strong>LabEx</strong></a> is a <strong>hands-on learning platform for beginners</strong>.</p><p>Explore <a href="https://labex.io/learn/linux"><strong>Linux</strong></a>, <a href="https://labex.io/learn/devops"><strong>DevOps</strong></a>, <a href="https://labex.io/learn/cybersecurity"><strong>Cybersecurity</strong></a>, and <strong>more</strong> — all directly in your browser.</p><p>Learn step by step through <strong>interactive labs</strong>, <strong>guided exercises</strong>, and <strong>real-world projects</strong>. 🌱<br />No setup, no stress — just practice and grow your skills by doing.</p><br /><p><a href="https://apps.apple.com/app/id6765840991"><img src="https://developer.apple.com/assets/elements/badges/download-on-the-app-store.svg" alt="Download on the App Store" height="54" /></a>&nbsp;<a href="https://apps.apple.com/app/id6765840991"><img src="https://developer.apple.com/app-store/marketing/guidelines/images/badge-download-on-the-mac-app-store.svg" alt="Download on the Mac App Store" height="52" /></a></p><br /><p>📖 Need help? Visit our <a href="https://support.labex.io/">Help Center</a> or email info@labex.io</p></div>

